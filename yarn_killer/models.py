...
class User(db.Model):
    ...
    stash = db.relationship(
        "Stash", backref="user", lazy=True, uselist=False, cascade="all, delete-orphan"
    )
...
class Yarn(db.Model):
    ...
    fibers = db.relationship(
        "Fiber", backref="yarn", lazy=True, cascade="all, delete-orphan"
    )
    links = db.relationship(
        "Link", backref="yarn", lazy=True, cascade="all, delete-orphan"
    )
    stores = db.relationship("Store", secondary=stores_table, back_populates="yarns")
    colorways = db.relationship(
        "Colorway", backref="yarn", lazy=True, cascade="all, delete-orphan"
    )
    images = db.relationship(
        "Image", backref="yarn", lazy=True, cascade="all, delete-orphan"
    )
...
class Image(db.Model):
    ...
    colorway_id = db.Column(db.Integer, db.ForeignKey("colorways.id"))
...
class Colorway(db.Model):
    ...
    images = db.relationship(
        "Image", backref="colorway", lazy=True, cascade="all, delete-orphan"
    )
...
class Stash(db.Model):
    ...
    stock_list = db.relationship(
        "Stock", backref="stash", lazy=True, cascade="all, delete-orphan"
    )
...
class Store(db.Model):
    ...
    links = db.relationship(
        "Link", backref="store", lazy=True, cascade="all, delete-orphan"
    )
...
class Link(db.Model):
    ...
    #         price = (
    #             soup.find("span", "product-prices__sell-price").text.strip().strip("$")
    #         )
    ...
    def extract_colorways(self, recheck=False):
        ...
        #     image = np.asarray(
        #         bytearray(requests.get(image_src, stream=True).raw.read()),
        #         dtype="uint8",
        #     )
        ...
        #         print(f"^^^ {color_name[0]}")
        #         (
        #             colorway.color_specific,
        #             colorway.color_medium,
        #             colorway.color_broad,
        #         ) = colors
        #         colorway.sanity_check_colors()
        #         db.session.commit()
